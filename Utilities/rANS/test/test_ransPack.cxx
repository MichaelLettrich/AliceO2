...
BOOST_AUTO_TEST_CASE(test_pack)
{
  ...
  for (size_t packingWidth = 1; packingWidth <= 63; ++packingWidth) {
    ...
    for (auto i : source) {
      o2::rans::internal::pack(iter, bitOffset, i, packingWidth);
    };

    std::vector<uint8_t> packingBuffer2(source.size() * sizeof(source_type), 0);

    // use buffer API
    auto packingBuffer2End = reinterpret_cast<uint8_t*>(o2::rans::internal::packStream(source.begin(),
                                                                                       source.end(),
...
BOOST_AUTO_TEST_CASE(test_packUnpackNext)
{
  ...
  for (size_t packingWidth = 1; packingWidth <= 63; ++packingWidth) {
    ...
    for (size_t i = 0; i < source.size(); ++i) {
      unpackBuffer.push_back(o2::rans::internal::unpackNext(unpackIter, bitOffset, packingWidth));
    };
...
BOOST_AUTO_TEST_CASE(test_packRUnpackNext)
{
  ...
  for (size_t packingWidth = 1; packingWidth <= 63; ++packingWidth) {
    ...
    std::vector<source_type> unpackBuffer;
    for (size_t i = 0; i < source.size(); ++i) {
      unpackBuffer.push_back(o2::rans::internal::rUnpackNext(iter, bitOffset, packingWidth));
    };
...
BOOST_AUTO_TEST_CASE(test_packUnpackByIdx)
{
  ...
  for (size_t packingWidth = 1; packingWidth <= 63; ++packingWidth) {
    ...
    for (size_t i = 0; i < source.size(); ++i) {
      unpackBuffer.push_back(o2::rans::internal::unpackByIndex(iter, i, packingWidth));
    };
...
BOOST_AUTO_TEST_CASE(test_packUnpackStream)
{
  ...
  for (size_t packingWidth = 1; packingWidth <= 63; ++packingWidth) {
    ...
    // use buffer API
    auto packingBufferEnd = reinterpret_cast<uint8_t*>(o2::rans::internal::packStream(source.begin(),
                                                                                      source.end(),
    ...
    // unpack full buffer
    o2::rans::internal::unpackStream(reinterpret_cast<uint64_t*>(packingBuffer.data()),
                                     unpackBuffer.data(),
                                     source.size(),
                                     packingWidth);
...
BOOST_AUTO_TEST_CASE(test_packRUnpackEliasDelta)
{
  ...
  for (size_t packingWidth = 1; packingWidth <= 32; ++packingWidth) {
    ...
    uint32_t bitOffset = 0;
    for (auto i : source) {
      o2::rans::internal::eliasDeltaEncode(iter, bitOffset, i);
    };

    std::vector<uint32_t> unpackBuffer;
    for (size_t i = 0; i < source.size(); ++i) {
      unpackBuffer.push_back(o2::rans::internal::eliasDeltaDecode(iter, bitOffset));
    };
