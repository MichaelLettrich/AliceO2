...
static void ransCompressionBenchmark(benchmark::State& st, fixture_T& fixture)
{
  ...
  for (auto _ : st) {
    ...
    benchmark::DoNotOptimize(encodeBufferEnd = fixture.encoder.process(sourceMessage.data(), sourceMessage.data() + sourceMessage.size(), encodeBuffer.data(), literalsBuffer));
  }
#ifdef ENABLE_VTUNE_PROFILER
  __itt_pause();
#endif
  decodeBuffer.clear();
  fixture.decoder.process(encodeBufferEnd, std::back_inserter(decodeBuffer), sourceMessage.size(), literalsBuffer);
