...
void ransEncodeDecode(const std::string& name, const std::vector<source_T>& inputData, rapidjson::Writer<rapidjson::OStreamWrapper>& writer)
{
  ...
  const auto [metrics, renormedHistogram] = t.timeAndLog("Renorming", "Renormed Frequency Table", [&]() mutable {
    Metrics<source_type> metrics{histogram};
    auto renormedHistogram = renorm(std::move(tmpHist), metrics);
    return std::tuple(std::move(metrics), std::move(renormedHistogram));
  });
