...
void pack(uint64_t*& __restrict__ buffer, uint32_t& bitOffset, uint64_t data, uint32_t packingWidth)
{
  ...
  if (newBitOffset <= I64Bits) {
    ...
    *buffer |= bitExtract(data, 0, leftTail) << bitOffset;
...
uint64_t unpackNext(const uint64_t*& __restrict__ buffer, uint32_t& bitOffset, uint32_t packingWidth)
{
  constexpr uint32_t I64Bits = internal::toBits(sizeof(uint64_t));
  const uint32_t bitEnd = bitOffset + packingWidth;
  if (bitEnd <= I64Bits) {
    uint64_t ret = bitExtract(*buffer, bitOffset, packingWidth);
    bitOffset = bitEnd;
    return ret;
  } else {
    // first part
    uint64_t value = bitExtract(*buffer, bitOffset, I64Bits - bitOffset);
    //second part
    bitOffset = bitEnd - I64Bits;
    value |= bitExtract(*(++buffer), 0, bitOffset) << (packingWidth - bitOffset);
    return value;
  }
};

uint64_t rUnpackNext(uint64_t*& __restrict__ buffer, uint32_t& bitOffset, uint32_t packingWidth)
{
  constexpr uint32_t I64Bits = internal::toBits(sizeof(uint64_t));

  if (packingWidth < bitOffset) {
    bitOffset -= packingWidth;
    return bitExtract(*buffer, bitOffset, packingWidth);
  } else {
    // first part
    const uint32_t remainderBits = packingWidth - bitOffset;
    uint64_t value = bitExtract(*buffer, 0, bitOffset) << remainderBits;
    //second part
    bitOffset = 64 - remainderBits;
    value |= bitExtract(*(--buffer), bitOffset, remainderBits);
    return value;
  }
};

inline uint64_t unpackByIndex(const uint64_t* __restrict__ buffer, size_t index, uint32_t packingWidth)
{
  constexpr uint32_t I64Bits = internal::toBits(sizeof(uint64_t));
...
void deltaEncode(uint64_t*& __restrict__ buffer, uint32_t& bitOffset, uint32_t data)
{
  ...
  value = value << highestPow2 | bitExtract(data, 0, highestPow2);
  uint32_t packingOffset = nLeadingZeros + nLeadingZeros + 1 + highestPow2;

  pack(buffer, bitOffset, value, packingOffset);
};

uint32_t deltaDecode(uint64_t*& __restrict__ buffer, uint32_t& bitOffset)
{
  constexpr uint32_t I64Bits = internal::toBits(sizeof(uint64_t));
  ...
  const uint32_t highestPow2 = bitExtract(rawData, I64Bits - packingOffset, nLeadingZeros + 1) - 1;
  packingOffset += highestPow2;
  uint32_t value = (1 << highestPow2) + bitExtract(rawData, I64Bits - packingOffset, highestPow2);
