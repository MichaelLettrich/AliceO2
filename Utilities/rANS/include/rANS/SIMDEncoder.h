...
class SIMDEncoder
{
 ...
 public:
  ...
  SIMDEncoder(encoderSymbolTable_t e, size_t symbolTablePrecision);
  SIMDEncoder(const FrequencyTable& frequencies, size_t symbolTablePrecision);

  inline size_t getSymbolTablePrecision() const noexcept { return mSymbolTablePrecision; }
...
SIMDEncoder<coder_T, stream_T, source_T, nStreams_V>::SIMDEncoder(encoderSymbolTable_t e, size_t symbolTablePrecision) : mSymbolTable{std::move(e)}, mSymbolTablePrecision{symbolTablePrecision} {};

template <typename coder_T, typename stream_T, typename source_T, uint8_t nStreams_V>
SIMDEncoder<coder_T, stream_T, source_T, nStreams_V>::SIMDEncoder(const FrequencyTable& frequencies, size_t symbolTablePrecision)
{
  internal::SymbolStatistics stats{frequencies, mSymbolTablePrecision};
  mSymbolTablePrecision = stats.getSymbolTablePrecision();
...
stream_IT SIMDEncoder<coder_T, stream_T, source_T, nStreams_V>::process(source_IT inputBegin, source_IT inputEnd, stream_IT outputBegin) const
{
  ...
  auto maskedEncode = [this](source_IT symbolIter, stream_IT outputIter, ransCoder_t& coder, size_t nActiveStreams = nHardwareStreams_V) {
    ...
    return std::tuple(symbolIter, coder.putSymbols(outputIter, encoderSymbols, this->mSymbolTablePrecision, nActiveStreams));
  };
  ...
  auto encode = [this](source_IT symbolIter, stream_IT outputIter, ransCoder_t& coder) {
    ...
    return std::tuple(symbolIter, coder.putSymbols(outputIter, encoderSymbols, this->mSymbolTablePrecision));
  };
