...
class Metrics
{
 ...
 private:
  template <template <class> class Histogram_T>
  void computeMetrics(const Histogram_T<source_type>& frequencyTable);
  void updateCoderProperties(size_t newRenormingPrecisionBits, source_type min, source_type max);
...
{
  const auto [min, max] = getMinMax(histogram);
  updateCoderProperties(histogram.getRenormingBits(), min, max);
};
...
inline void Metrics<source_T>::updateCoderProperties(const SymbolTable<source_type, symbol_T>& symbolTable)
{
  ...
  updateCoderProperties(symbolTable.getPrecision(), min, max);
};

template <typename source_T>
template <template <class> class Histogram_T>
void Metrics<source_T>::computeMetrics(const Histogram_T<source_T>& histogram)
{
  using namespace internal;
  ...
  for (size_t i = 0; i < trimmedFrequencyView.size(); ++i) {
    ...
    if (frequency) {
      ...
      ++mDatasetProperties.symbolLengthDistribution[static_cast<uint32_t>(-length)];
...
template <typename source_T>
inline void Metrics<source_T>::updateCoderProperties(size_t newRenormingPrecisionBits, source_type min, source_type max)
{
  mCoderProperties.renormingPrecisionBits = internal::sanitizeRenormingBitRange(newRenormingPrecisionBits);
  mCoderProperties.nIncompressibleSymbols = internal::computeNIncompressibleSymbols<>(mDatasetProperties.weightedSymbolLengthDistribution.begin(),
                                                                                      mDatasetProperties.weightedSymbolLengthDistribution.end(),
