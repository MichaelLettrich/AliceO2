...
void DCAFitterN<N, Args...>::calcPCANoErr()
{
  ...
  o2::math_utils::rotateZ<double>(mTrPos[mCurHyp][N - 1][0], mTrPos[mCurHyp][N - 1][1], pca[0], pca[1], mTrAux[N - 1].s, mTrAux[N - 1].c);
  //RRRR    mTrAux[N-1].loc2glo(mTrPos[mCurHyp][N-1][0], mTrPos[mCurHyp][N-1][1], pca[0], pca[1] );
  pca[2] = mTrPos[mCurHyp][N - 1][2];
  for (int i = N - 1; i--;) {
    double x, y;
    o2::math_utils::rotateZ<double>(mTrPos[mCurHyp][i][0], mTrPos[mCurHyp][i][1], x, y, mTrAux[i].s, mTrAux[i].c);
...
void DCAFitterN<N, Args...>::calcTrackResiduals()
{
  ...
  for (int i = N; i--;) {
    ...
    vtxLoc = mPCA[mCurHyp];
    o2::math_utils::rotateZInv<double>(vtxLoc[0], vtxLoc[1], vtxLoc[0], vtxLoc[1], mTrAux[i].s, mTrAux[i].c); // glo->loc
