...
bool MatchTPCITS::prepareTPCTracks()
{
  ...
  for (int it = 0; it < ntr; it++) {
    ...
    mTPCSectIndexCache[o2::math_utils::angle2Sectorf(trc.getAlpha())].push_back(mTPCWork.size() - 1);
  }
...
bool MatchTPCITS::prepareITSTracks()
{
  ...
  for (int irof = 0; irof < nROFs; irof++) {
    ...
    for (int it = rofRec.getFirstEntry(); it < trlim; it++) {
      ...
      if (!trc.rotate(o2::math_utils::angle2Alphaf(trc.getPhiPos()))) {
      ...
      // cache work track index
      int sector = o2::math_utils::angle2Sectorf(trc.getAlpha());
...
void MatchTPCITS::addLastTrackCloneForNeighbourSector(int sector)
{
  ...
  if (trc.rotate(o2::math_utils::sector2Anglef(sector)) &&
...
bool MatchTPCITS::propagateToRefX(o2::track::TrackParCov& trc)
{
  ...
  while (o2::base::Propagator::Instance()->PropagateToXBxByBz(trc, XMatchingRef, o2::constants::physics::MassPionCharged,
    ...
    auto alphaNew = o2::math_utils::angle2Alphaf(trc.getPhiPos());
...
bool MatchTPCITS::refitTrackTPCITSloopITS(int iITS, int& iTPC)
{
  ...
  auto& tracOut = trfit.getParamOut(); // this track is already at the matching reference X
  {
    ...
    if (!tracOut.rotate(o2::math_utils::sector2Anglef(sector % 18))) {
...
bool MatchTPCITS::refitTPCInward(o2::track::TrackParCov& trcIn, float& chi2, float xTgt, int trcID, float timeTB, float m) const
{
  ...
  if (!trcIn.rotate(o2::math_utils::sector2Anglef(sectCurr % 18))) {
...
int MatchTPCITS::checkABSeedFromLr(int lrSeed, int seedID, ABTrackLinksList& llist)
{
  ...
  float xCurr, yCurr;
  o2::math_utils::rotateZf(seed.getX(), seed.getY(), xCurr, yCurr, sna, csa);
