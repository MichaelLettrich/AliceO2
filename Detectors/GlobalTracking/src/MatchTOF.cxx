...
bool MatchTOF::prepareTracks()
{
  ...
  for (int it = 0; it < mNumOfTracks; it++) {
    ...
    LOG(DEBUG) << "The track will go to sector " << o2::math_utils::angle2Sectorf(TMath::ATan2(globalPos[1], globalPos[0]));

    mTracksSectIndexCache[o2::math_utils::angle2Sectorf(TMath::ATan2(globalPos[1], globalPos[0]))].push_back(it);
...
bool MatchTOF::prepareTPCTracks()
{
  ...
  for (int it = 0; it < mNumOfTracks; it++) {
    ...
    LOG(INFO) << "The track will go to sector " << o2::math_utils::angle2Sectorf(TMath::ATan2(globalPos[1], globalPos[0]));
#endif
...
bool MatchTOF::propagateToRefXWithoutCov(o2::track::TrackParCov& trc, float xRef, float stepInCm, float bzField)
{
  ...
  while (hasPropagated) {
    ...
    if (fabs(trcNoCov.getY()) > trcNoCov.getX() * tanHalfSector) { // we are still in the same sector
      // we need to rotate the track to go to the new sector
      //Printf("propagateToRefX: changing sector");
      auto alphaNew = o2::math_utils::angle2Alphaf(trcNoCov.getPhiPos());
