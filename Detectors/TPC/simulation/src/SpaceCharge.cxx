  ...
  if (mSCDistortionType == SCDistortionType::SCDistortionsRealistic) {
    ...
    for (int iside = 0; iside < 2; ++iside) {
      ...
      for (int iphi = 0; iphi < mNPhi; ++iphi) {
        ...
        int roc = iside == 0 ? o2::math_utils::angle2Sectorf(phi) : o2::math_utils::angle2Sectorf(phi) + 18;
        for (int ir = 0; ir < mNR; ++ir) {
      ...
      for (int ir0 = 0; ir0 < mNR; ++ir0) {
        ...
        for (int iz0 = 0; iz0 < mNZ; ++iz0) {
          ...
          for (int iion = 0; iion < nIons; ++iion) {
            ...
            o2::math_utils::bringTo02PiGenf(phiIonF);
            rIon += dr;
            zIon += dz;
...
void SpaceCharge::correctElectron(GlobalPosition3D& point)
{
  ...
  float phi = point.phi();
  o2::math_utils::bringTo02PiGenf(phi);
  int roc = o2::math_utils::angle2Sectorf(phi);
...
double SpaceCharge::getChargeDensity(Side side, const GlobalPosition3D& point) const
{
  ...
  o2::math_utils::bringTo02PiGenf(x[1]);
  const int roc = side == Side::A ? o2::math_utils::angle2Sectorf(x[1]) : o2::math_utils::angle2Sectorf(x[1]) + 18;
